  ...
  etcd:
    local:
      dataDir: "/tmp/etcd"
        ...
        sh('''/home/centos/workspace/scale_client_clusters/kind create cluster --name group-1-12--cls-'''+ uuid1 +''' --config /home/centos/workspace/scale_client_clusters/kind.config''')

        // def uuid2 = Math.abs(new Random().nextInt() % 1200000) + 1
        // sh('''/home/centos/workspace/scale_client_clusters/kind create cluster --name group-1-12--cls-'''+ uuid2 + ''' --config /home/centos/workspace/scale_client_clusters/kind.config''')
        ...
        // // staging-0
        sh('''docker exec -i execution /bin/bash -c "cd /home/centos/workspace/scale_client_clusters && export ONBOARD=true && export WORKSPACE=/home/centos/workspace/scale_client_clusters && python3.7 library/pyfra.py --tests-dir setup/client_cluster --cluster-type kind --clusters group-1-12--cls-''' + uuid1+''' --log-dir . --debug --csp-token OgyamW2imj6L2CKRq7RcLj4ag9S2AZXrRdBCNj02XY11r8daNXWIY9egDi9bMFw6 --clusters-per-tenant 1 --apps-per-cluster 17"''')        
