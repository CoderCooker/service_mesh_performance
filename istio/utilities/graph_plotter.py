...
# Description: visualize tests results module for service mesh performance
...
metric_dict = {"cpu-istio-ingressgateway": "cpu_mili_avg_istio_proxy_istio-ingressgateway",
               "mem-istio-ingressgateway": "mem_Mi_avg_istio_proxy_istio-ingressgateway"}
...
def Run(args):
    try:
        #argv = sys.argv[6:]
        argv = ["--graph_type=latency-p99", "--x_axis=conn", "--telemetry_modes=telemetryv2_both,telemetryv2_baseline", "--query_list=2,16", "--query_str=ActualQPS==1000", "--csv_filepath=test_data/10PODS_SC.csv", "--graph_title=p99.png"]
        args.log.info("start testing %s passed in parameters %s" % (args.shortName, argv))
        processed_args = get_parser().parse_args(argv)
        args.log.info("processed parameters {}".format(processed_args))
