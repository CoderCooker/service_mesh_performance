  ...
  etcd:
    local:
      dataDir: "/tmp/etcd"
        ...
        sh('/home/centos/workspace/exhaust-masterkind get clusters | xargs /home/centos/workspace/exhaust-master/kind delete clusters')
       
        
        def uuid1 = Math.abs(new Random().nextInt() % 500000) + 1
        sh('''/home/centos/workspace/exhaust-master/kind create cluster --name cls-1-5-'''+ uuid1 +''' --config /home/centos/workspace/exhaust-master/kind.config''')

        // def uuid2 = Math.abs(new Random().nextInt() % 1200000) + 1
        // sh('''/home/centos/workspace/exhaust-master/kind create cluster --name cls-1-5-'''+ uuid2 + ''' --config /home/centos/workspace/exhaust-master/kind.config''')
        ...
        // two clusters
        //sh('''docker exec -i execution /bin/bash -c "cd /home/centos/workspace/exhaust-master && export ONBOARD=true && export WORKSPACE=/home/centos/workspace/exhaust-master && python3.7 library/pyfra.py --tests-dir setup/client_cluster --cluster-type kind --clusters cls-1-5-''' + uuid1+''',cls-1-5-'''+uuid2+''' --log-dir . --debug --csp-token OgyamW2imj6L2CKRq7RcLj4ag9S2AZXrRdBCNj02XY11r8daNXWIY9egDi9bMFw6 --clusters-per-tenant 1 --apps-per-cluster 17"''')

        // // staging-0 f55
        sh('''docker exec -i execution /bin/bash -c "cd /home/centos/workspace/exhaust-master && export ONBOARD=true && export WORKSPACE=/home/centos/workspace/exhaust-master && python3.7 library/pyfra.py --tests-dir setup/client_cluster --cluster-type kind --clusters cls-1-5-''' + uuid1+''' --log-dir . --debug --csp-token P8ewTR1jva1zc8y7g3JyvXJyoit7Xodfzj7QFNnLs66YOKDCDIWSznFu4dbrrWyv --clusters-per-tenant 1 --apps-per-cluster 17"''')        
